...
export default function DashProfile() {
  ...
  // console.log("dash profile : ",currentstudent);
      ...
      const res = await axios.post(
        ...
        payload,
        {
          onUploadProgress: (e) => {
            ...
            setimageFileUploadProgress(pct);
          },
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:3000/server/student/update/${currentstudent._id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `http://localhost:3000/server/student/delete/${currentstudent._id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
  ...
  const handleSignout = async () => {
    try {
      const isAlumni = currentstudent?.isAlumni;
      const res = await fetch("http://localhost:3000/server/student/signout", {
        method: "POST",
        credentials: "include",
      });
      const data = await res.json();
