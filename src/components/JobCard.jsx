...
const JobCard = ({ job }) => {
  ...
  const maxWords = 25;
  const words = job.requireskills
    ? job.requireskills.split(/\s+/)
    : [];
  const isLong = words.length > maxWords;

  const displaySkills = !expanded && isLong
  ...
  return (
    <div className="border-2 rounded-lg p-6 mb-4 border-custom-blue relative dark:text-white transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover: duration-300 ..." >
      ...
      <p className=" mb-2"><span className="font-medium">Required Skills:</span> {displaySkills}
        {isLong && (
          <button
