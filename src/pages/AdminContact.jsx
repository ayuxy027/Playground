...
const ContactList = () => {
    ...
    const fetchContacts = async () => {
      try {
        const response = await fetch(
          "http://localhost:3000/server/contact/displaycontact",
          { credentials: "include" }
        );
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `http://localhost:3000/server/contact/deletecontact/${id}`,
        {
          method: "DELETE",
          credentials: "include",
        }
      ...
      {contacts.length > 0 ? (
        <ul className="space-y-4">
          {contacts.map((contact) => (
            <li
              key={contact._id}
              className="bg-white border-2 rounded-lg p-6 mb-4 border-custom-blue"
            >
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-gray-600 mb-2">
                    Issue: {contact.issueDescription}
                  </p>
                </div>
                <button
